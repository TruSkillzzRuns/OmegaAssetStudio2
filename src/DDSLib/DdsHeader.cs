...
namespace DDSLib
{
    public sealed class DdsHeader
    {
        ...
        public DdsHeader(DdsSaveConfig config, int width, int height, int mipMaps = 1)
        {
            ...
            Dx10Header = config.FileFormat == FileFormat.BC7
                ? new DdsHeaderDx10(98)
                : null;

            bool isCompressed = config.FileFormat == FileFormat.DXT1
                             || config.FileFormat == FileFormat.DXT3
                             || config.FileFormat == FileFormat.DXT5
                             || config.FileFormat == FileFormat.BC5
                             || config.FileFormat == FileFormat.BC7;
            //
        ...
        public DdsHeaderDx10 Dx10Header { get; private set; }
        ...
        public void Read(BinaryReader reader)
        {
            ...
            if (PixelFormat.FourCC == FourCCFormat.Dx10)
            {
                Dx10Header = new DdsHeaderDx10();
                Dx10Header.Read(reader);
            }
        }
